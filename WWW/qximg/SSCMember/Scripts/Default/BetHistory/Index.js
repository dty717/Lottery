...
define(function (require, exports, module) {
    var AccountList = function (params) {
        ...
        this.HistoryToBetList = function ($data) {
            framework.view("/BetList/Index", "BetList/index", { PeriodsNumber: $data.PeriodsNumber, IsHistoryList:true});
        }
        this.goReport = function () {//返回报表
            var ReportStatus = that.ReportStatus()-0;
            if (ReportStatus == 0)
            {
                framework.view('/Report/DayReport', 'Report/DayReport');
            }
            else if (ReportStatus == 1)
            {
                framework.view('/Report/WeekReport', 'Report/WeekReport', { LastMonth: this.LastMonth() });
            }
            else if (ReportStatus == 2)
            {
                
                framework.view('/Report/MonthReport', 'Report/MonthReport', { LastMonth: this.LastMonth() });
            }
        }
        this.init();
    }
    ...
    AccountList.prototype.GetData = function () {  
        ...
        /*用户查询历史账单*/
        $.ajax({
            url: "/BetHistory/GetHistoryBetInfo",
            data: {
                lotteryId: 1,
