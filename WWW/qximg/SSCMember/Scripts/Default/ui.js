...
define(function (require, exports, module) {
    function randomTime() {
        return 1000 * 90  + Math.floor(Math.random() * 10000) % 30 * 1000;
    }
    exports.StopNumber = ko.observableArray();
    exports.childList = ko.observableArray();
    exports.menuName = ko.observable('规则说明');
    exports.tabmenu = function (url, js, $index, $data, event, isAutoClick) {
        ...
        /*  点击非快打按钮，重置快打状态。
            便于开盘封盘时，快打页面请求的接口判断状态。
            因为开盘 <-->封盘时候，刷新页面是靠触发 tab click事件刷新的页面。
            当 FastBeatIsInit 为 1时，说明页面加载过了，此时代码自动刷新页面时，有些接口是不触发的。
            当 FastBeatIsInit 为 0时，接口都照常请求。
        */
        if(ele.attr('href') !== "/BetPanel/FastBeat/Index" || !isAutoClick) {
            framework.temp.FastBeatIsInit = 0;
            framework.temp.notFirstTime = 0;
        }
        ...
        framework.temp.isFromCloseToOpen = false;
    ...
    */
    ...
    exports.refreshWhenOpen = function (isForce) {
        ...
        $li.children("a").trigger("click", true, 1);
    }
    ...
    */
    ...
    $(function () {
        ...
        var info = $('.stoptime'),
            noticeSettings = {
                url: "/Home/GetUserLoginNoticeByType",
                cache: false,
                dataType: "json",
                data: {}
            };
        ...
        var periods = {
            ...
            main: function (fn) {
                var that = this;
                $.ajax({ url: '/Home/GetLastPeriods', cache: false }).done(function (data) {
                    ...
                    var oldPeriodStatus = Utils.Cookie.get('PeriodsStatus');
                    if (data.PeriodsStatus !== oldPeriodStatus) {
                    ...
                    /// 缓存最新期数和id
                    framework.temp.latestPeriod = {
                        period: data.PeriodsNumber,
                        id: data.ID
                    };
                    ...
                    function next(d) {
                        /// 是否从封盘状态转变成开盘状态
                        framework.temp.isFromCloseToOpen = oldPeriodStatus !== null && oldPeriodStatus - 0 === 0 && d.PeriodsStatus - 0 === 1;
                        ...
                        periods.calcDiff(time, d.nowDateTime, function (day, hours, minute, seconds, flags) {
                            ...
                            if (flags) {
                                ...
                                if (periods.periodsStatus !== tempvalue && periods.getInterval != null) {
                                    ...
                                    // 一分半到两分
                                    periods.getInterval = setInterval(periods.main, randomTime());
                                }
                            }
                    ...
                    if (currentStatus !== status) {
                        ...
                        if (framework.currentItem[0] && !framework._extend.NotAjax) {
                            ...
                            if (framework.container().indexOf("FastBeat/Index") < 0) {
                                setTimeout(function () {
                                    framework.currentItem[0].trigger('click', true, 1);
                                }, 0);
                            }
                        }
                    }
        ...
        var notice = {
            ...
            noticeFactory: {
                marquee: function (element, content, endTime) {
                    var
                        element = $(element);
                    element.html(element.html() + content[0].Detail);
            ...
            main: function () {
                ...
                $.ajax(noticeSettings)
                    .done(function (data) {
						console.log(data)
                        var tick = data.AnnouncementList;
                        if (tick && tick[1].length) {
                            Utils.Cookie.set('Ticks', noticeSettings.data.Ticks = tick.shift(), 0, '/');
                            //跑马灯,弹窗,右下角
                            var marquee = [],
                                popup = [],
                                right = [],
                                ...
                                type,
                                tick = tick[0];
                            for (var i = 0, len = tick.length; i < len; i++) {
                                type = tick[i]['ShowType'];
                                switch (type) {
                                    case 1:
                                        marquee.push(tick[i]);
                                        if (i === tick.length - 1) {
                                            endTime = tick[i].EndDt;
                                        }
                                        break;
                                    case 2:
                                        popup.push(tick[i]);
                        ...
                        if(info) {
                            var c_t = info.c_t;
                            var c_r = info.c_r;
                            var session_c_t = Utils.Cookie.get('c_t');

                            if(session_c_t == null) {
                                Utils.Cookie.set('c_t', c_t);
                            } else {
                                if(session_c_t !== c_t) {
                                    Utils.Cookie.set('c_t', c_t);
