...
define(function (require, exports, module) {
    ...
    var AccountList = function () {
        ...
        //this.periodsStatus = ko.observable(true)
	console.log(Utils.Cookie.get('PeriodsID'),Utils.Cookie.get('PeriodsStatus'));
	// !!Utils.Cookie.get('PeriodsID') || Utils.Cookie.get('PeriodsStatus') - 0 !== 0;     //期数状态,如果为true，则为开盘
        ...
        this.deleteStopNumber = function () {/***********压停号码删除**********************************************************/
            ...
            if (arr.length == 0) {
                ...
                var that = this;
                $.ajax({
                    url: "/FastBeat/DeleteStopBetNumber",
                    type: "post",
                    data: {
        ...
        this.ConfirmTheBet = function () {
            ...
            var status =1// Utils.Cookie.get('PeriodsStatus'),//期数状态
            ...
            if (typeof number === 'undefined' || number === '') {
                ...
                if (status == '1') {
                    ...
                    var p=makeStruct();
                    var rules=[]
                    if(that.sizixian()||that.quanZhuang()){
                        全转参数=[];
                        for(var str in $('#NumberFastBeat').val()){
                            if(makeValid(str)!="x")
                                全转参数.push(parseInt(str));
                        }
                        rules.push(全转);    
                    }
                    p.rule=rules;
                    
                    getResults(4,p.set())[0].forEach(function(elem){
                         if(elem[4]=="x"){
                            console.log(elem.substring(0,4))
                         }  
        ...
        this.GetTopEightPeriodsNumber = function () {
            ...
            /**查询停压号码:这里返回二类数据，包含删除数据*******************************/
            var ajaxRequest = $.ajax({
                url: "/FastBeat/GetTopEightPeriodsNumber",
                cache: false,
                dataType: "json", 
                success: function (json) {
    ...
    AccountList.prototype.init = function (fn) {
        ...
        if (!framework._extend.NotAjax && !isInit) {
            that.GetData(fn);
        } else {
            that.renderIntData();
        }
    ...
    AccountList.prototype.StopBetNumber = function (fn) {
        ...
        if (framework.temp.isFromCloseToOpen) {
            ...
            $.ajax({
                url: "/FastBeat/GetPeriodsNumberByStop",
                cache: false,
                dataType: "json",
                success: function (data) {
