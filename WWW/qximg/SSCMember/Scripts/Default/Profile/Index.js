...
define(function (require, exports, module) {
     // 定位id 枚举值
    var fixEnum = {
        oneFix: [20, 21, 22, 23, 24],
        twofix: [2, 3, 4, 5, 6, 7, 17, 18],
        threeFix: [9, 10, 11, 12],
        fourFix: [13],
        TwoDisplay: [14],
        ThreeDiplay: [15],
    ...
    var AccountList = function (params) {
        ...
        this.sub = function ($form) {
            ...
            $.ajax({
                url: '/PeriodsManage/ModifyMemberBetTypeSetting',
                type: "post",
                data: d,
                success: function (json) {
    ...
    AccountList.prototype.GetData = function (id) {
        ...
        //}
        $.ajax({
            url: "/PeriodsManage/GetMemberCommission", //获取限额列表信息
            data: { isMember: true },
            cache: false,
            dataType: "json",
            success: function (json) {
                ...
                var sortedJson = [];
                for(var type in fixEnum){
                    var betType = fixEnum[type]
                    sortedJson = sortedJson.concat(json.filter(function (item) {
                         return betType.indexOf(item.BetTypeID) != -1
                    }))
                }
                var dataArr = [], dataObj = {}, defaultsel = [];
                ko.utils.arrayForEach(sortedJson, function (element) {
    ...
    AccountList.prototype.back = function () {
        var that = this,
            params = that.params;
        if (params.isMember) {
            framework.view("/Member/index", "Member/index", { AgentID: params.ParentID, history: params.history, ParentID: params.MemberParentID });
        }
        else {
