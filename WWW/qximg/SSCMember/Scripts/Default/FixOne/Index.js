...
define(function (require, exports, module) {
    ...
    function arrayIntersection(a, b){
        ...
        var lang = a.length > b.length ? a : b;

        var result = [];
        short.forEach(function(item, index) {
            lang.forEach(function(langItem){
                if(langItem.BetTypeName === item.BetTypeName) {
                    result.push(item)
                }
            })
        })
    ...
    var FixTwo = function (param) {
        ...
        ///初始化数据
         $.post("/FixTwo/CheckCredit", function (data) {
            if (data.status) {
                that.credit(true);
            }
        ...
        this.addHeFen = function (data, event) {
            ...
            if(num === '重') {
                var index = hefenNums.indexOf('无');
                if ( index !== -1 ) {
                    hefenNums.splice(index, 1);
                    $('#HeFen [value="无"]').removeClass('btred');
                }
            } else if (num === '无') {
                var index = hefenNums.indexOf('重');
                if( index !== -1 ) {
                    hefenNums.splice(index, 1);
                    $('#HeFen [value="重"]').removeClass('btred');    
                }
            }
            ...
            $.each(list, function (index, value) {
                var number = value.BetTypeName.replace(/X/ig, '').split('');
                var sum = (+number[0]) + (+number[1]);

                for(var i = 0; i < hefenNums.length; i++) {
                    var hi = hefenNums[i];
                    var isBreak = false;

                    // 数字，单双判断
                    if (sum % 10 === hi || (hi === '单' && sum % 2 === 1) || (hi === '双' && sum % 2 === 0)) {
                        other.push(value);
                        isBreak = true;
                    }

                    // 重判断
                    if((hi === '重' && number[0] === number[1]) || (hi === '无' && number[0] !== number[1])){
                        double.push(value);
                        isBreak = true;
                    }
                }
            ...
            var result = double.length === 0 ? other : arrayIntersection(double, other.length === 0 ? list : other);

            result.forEach(function (item) {
                item.selectStyle(baseStyle)
            })
		...
		$.each(selectList, function (index, value) {
                    rules.push(stringToRule(value.BetTypeName))
                });    
		p.ruleOr=rules
		var sets=getResults(1,p.set())[0];
		
		//console.log(sets)
		var valueOdds
		if (selectList.length > 0) {
			valueOdds=selectList[0].Odds
		}
		...
		sets.forEach(function(item) {
			if(item[4]=="x"||(!item[4])){
				var data = {
                        		BetNumber: item.substring(0,4),/// 投注号
	    ...
	    $.each(list, function (index, value) {
		    ...
		    var p=makeStructByString(value.BetTypeName);					
		    //p.ruleOr=stringToRule(value.BetTypeName);
		    var sets=getResults(1,p.set())[0];
